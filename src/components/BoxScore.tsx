...
export default function BoxScore({ eventId, league, seattleTeamId, color = "#00d4ff" }: Props) {
  ...
  return (
    <div className="mt-1">
      <div className="font-display text-[10px] font-700 uppercase tracking-widest text-zinc-600 px-5 pt-4 pb-2">Box Score</div>
      ...
      {periodLabels.length > 0 && (
        <div className="px-3 overflow-x-auto no-scrollbar">
          <table className="w-full min-w-max text-right" style={{ borderCollapse: "separate", borderSpacing: 0 }}>
            <thead>
              <tr>
                <td className="text-left pr-2 pb-1 font-display text-[10px] text-zinc-600 uppercase tracking-wider w-10"></td>
                {periodLabels.map((label, i) => (
                  <td key={i} className="pb-1 px-1.5 font-display text-[10px] font-600 text-zinc-500 uppercase tracking-wider min-w-[28px]">{label}</td>
                ))}
                {isBaseball ? (
                  <>
                    <td className="pb-1 pl-2 pr-1 font-display text-[10px] font-700 text-zinc-400 uppercase border-l border-white/10">R</td>
                    <td className="pb-1 px-1 font-display text-[10px] font-600 text-zinc-500 uppercase">H</td>
                    <td className="pb-1 px-1 font-display text-[10px] font-600 text-zinc-500 uppercase">E</td>
                  </>
                ) : (
                  <td className="pb-1 pl-2 font-display text-[10px] font-700 text-zinc-300 uppercase border-l border-white/10">F</td>
                )}
              </tr>
            </thead>
            <tbody>
              {linescores.map((team, ti) => {
                ...
                return (
                  <tr key={team.teamId} className={ti === 0 ? "" : ""}>
                    {/* Team abbr + logo */}
                    <td className="pr-2 py-1.5 text-left">
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className={`font-display text-[12px] font-700 ${isSea ? "text-white" : "text-zinc-400"}`}>{team.abbr}</span>
                      </div>
                    </td>
                    {/* Per-period scores */}
                    {periodLabels.map((_, pi) => {
                      ...
                      return (
                        <td key={pi} className="px-1.5 py-1.5 font-display text-[12px] font-600 tabular-nums" style={{ color: val !== undefined ? (isSea ? "#e4e4e7" : "#71717a") : "#3f3f46" }}>
                          {val !== undefined ? val : "–"}
                        </td>
                      )
                    })}
                    ...
                    {isBaseball ? (
                      <>
                        <td className="pl-2 pr-1 py-1.5 font-display text-[14px] font-800 tabular-nums border-l border-white/10" style={{ color: isSea ? "#fff" : "#a1a1aa" }}>{rhe!.R}</td>
                        <td className="px-1 py-1.5 font-display text-[12px] font-600 tabular-nums text-zinc-400">{rhe!.H}</td>
                        <td className="px-1 py-1.5 font-display text-[12px] font-600 tabular-nums text-zinc-500">{rhe!.E}</td>
                      </>
                    ) : (
                      <td className="pl-2 py-1.5 font-display text-[14px] font-800 tabular-nums border-l border-white/10" style={{ color: isSea ? "#fff" : "#a1a1aa" }}>{Math.round(team.score)}</td>
                    )}
                  </tr>
                )
              })}
      ...
      {stats.length >= 2 && highlightKeys.length > 0 && (() => {
        // Find stats that both teams have
        ...
        const teamB = stats[1]
        return (
          <div className="px-3 mt-4 space-y-1.5 pb-2">
            <div className="font-display text-[10px] font-700 uppercase tracking-widest text-zinc-600 px-2 pb-1">Team Stats</div>
            {sharedStats.map(key => {
              ...
              return (
                <div key={key} className="px-2 py-1">
                  <div className="flex justify-between mb-1">
                    <span className="font-display text-[11px] font-700 text-white tabular-nums">{vA}</span>
                    <span className="font-display text-[10px] text-zinc-500 uppercase tracking-wide">{label}</span>
                    <span className="font-display text-[11px] font-700 text-zinc-300 tabular-nums">{vB}</span>
                  </div>
